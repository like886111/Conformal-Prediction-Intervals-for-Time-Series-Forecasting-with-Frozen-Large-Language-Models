...
def nll(input_arr, target_arr, model, settings:SerializerSettings, transform, count_seps=True, prompt=None, temp=1):
    ...
    response = _get_openai_client().completions.create(
        model=model, prompt=prompt, logprobs=5, max_tokens=0, echo=True, temperature=temp
    )
    choice = response.choices[0]
    logprobs = np.array(choice.logprobs.token_logprobs, dtype=np.float32)
    tokens = np.array(choice.logprobs.tokens)
    top5logprobs = choice.logprobs.top_logprobs
